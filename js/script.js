            ...
            response.json().then(function (data) {
                ...
                for (var i = 0; i <= nom.length - 1; i++) {
                    if (nom[i] === '') {
                      ...
                      resultat += "<div class='accordion' id='accordionExample'><div class='card'><div class='card-header' id='heading"
                      + [i] +"'><h2 class='mb-0'><button class='btn btn-link' type='button' data-toggle='collapse' data-target='#collapse"
                      + [i] +"' aria-expanded='false' aria-controls='collapse" + [i] +"'>"
                      + nom[i] + "</button></h2></div><div id='collapse" + [i] +"' class='collapse' aria-labelledby='heading"
                      + [i] +"' data-parent='#accordionExample'><div class='card-body'>"
